...
authores: "Andrea Caterin Diaz Montealegre - Juan Sebastián Castro Salamanca"
date: "`r Sys.Date()`"
output: 
  html_document:
    toc: true
...
muestrastring <- muestrastring %>%
  ...
  mutate(COLUMNAS_CON_O = apply(., 1, function(fila) {
    paste(names(muestrastring)[fila == "O"], collapse = ", ")
  }))
...
muestrastring %>%
  ...
  select(DOCUMENTO, NIVEL_REAL, TOTAL_OMISIONES, COLUMNAS_CON_O) %>%
  adorn_totals("row") %>%
  flextable() %>%
  fontsize(size = 12, part ="all") %>%
  autofit() %>%
...
muestrastring <- muestrastring %>%
  mutate(TOTAL_MULTIMARCA = rowSums(. == "M", na.rm = TRUE)) %>%
  mutate(COLUMNAS_CON_M = apply(., 1, function(fila) {
    paste(names(muestrastring)[fila == "M"], collapse = ", ")
  }))


TOTAL_MULTIMARCA <- sum(muestrastring$MULTIMARCA)

muestrastring %>%
  filter(TOTAL_MULTIMARCA > 1) %>%
  select(DOCUMENTO, NIVEL_REAL, TOTAL_MULTIMARCA, COLUMNAS_CON_M) %>%
  adorn_totals("row") %>%
  flextable() %>%
  fontsize(size = 12, part ="all") %>%
  autofit() %>%
